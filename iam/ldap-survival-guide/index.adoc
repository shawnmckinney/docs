...
If you are an old LDAP engineer, you will quite certainly not like my point of view.
...
smckinn: It is technically correct to say there is no session from the client perspective. Although, it has connections, which function as a sessions WRT auditing requirements.
Was LDAPv3 designed with "Identity" in mind? No, of course not. As you point out it predates those concerns. 
Can it be used as the backbone for a "modern" Identity server/network? No answer needed. Just have a look around.
...
smckinn: LDAP is a victim of its own success. During its rapid adoption many mistakes were made in terms of the adopters. What we're talking about here is schema.
Specifically, inetorgperson and eduperson. Which was adopted by the enterprise and universities. These schemas contain multivalue attributes that should not have been.
It would be a simple matter to devise a new schema for modern applications. Perhaps that is somethign we should do.
...
smckinn: I've never seen a valid use case for a multivalued uid. Still waiting.
...
smckinn: Are we still conflating convention with technology? Why must we follow these unwritten conventions? Are we not in control of our own envs? Show me a use case where LDAP "can't" follow standard that doesn't involve a client not willing to change how they handle the data model.
...
smckinn: Here we agree again. But this is not necessarily so. Are we blaming the technology for mistakes made by its adopters?
...
smckinn: How about using delete? Why must the account remain if it has been (permanently) disabled? If it must be restored or interrogated move it to another ou. As you pointed out, LDAP is not an Identity system. It's a system of record and AuthN provider.
...
smckinn: Agreed, use memberOf for reverse group lookup.
...
smckinn: What we're talking about is schema. For example:
...
Why was it designed this way? Who knows? It's from the distant past. Yet, we continue to follow it today. How about design a new schema for groups? What's stopping you? (Bracing for "users")

***

This means that groups cannot be pre-provisioned.

Sure it can. Design a group schema that allows it.
...
smckinn: Hyperboloic statements aside lets look at the facts. The problems your describing have to do with the schema and not the technology. How it has been practiced. We can do better. Lets design and implement new schemas. Ones that support new use cases in Identity. Because as you've said, these data designs were built before even mobile computing. In short the data structures are outdated and in need of repair. The scaffolding, the LDAPv3 standard itself remains intact and capable of carrying the load.
